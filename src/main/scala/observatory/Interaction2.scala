...
object Interaction2 extends Interaction2Interface {
  ...
  def availableLayers: Seq[Layer] = {
    List(Layer(LayerName.Temperatures, tempColors, fromYear to toYear),
      Layer(LayerName.Deviations, devColors, devYear to toYear))
  }
  ...
  def yearSelection(selectedLayer: Signal[Layer], sliderValue: Signal[Year]): Signal[Year] = Signal {
    val b = selectedLayer().bounds
    sliderValue().max(b.start).min(b.end)
  }
